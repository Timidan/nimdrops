...
- **Expiry is a window the sponsor chooses, counted from finalized activation** and not from when you started the draft. It is between 1 hour and 14 days, defaulting to 24 hours; the floor stops a drop expiring before anyone could realistically open the link, and the ceiling bounds how long the operator holds a sponsor's NIM given that nothing can end a drop early. The value is fixed when the draft is created, applied at activation, and no request body can change it afterwards (migration `016`). At expiry the drop stops accepting new claims, every already-reserved claim is still honoured, and exactly one refund is created for the unallocated value: `(claim_count − reserved_claims) × amount_each`. The refund goes to the address that sent the verified funding transaction and to no other address. No request body can change it.
