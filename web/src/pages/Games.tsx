  ...
  return (
    <Screen>
      <div className="flex flex-1 flex-col bg-paper px-5 pt-9 pb-12 text-ink">
        ...
        {error !== null ? (
          <p data-testid="games-error" className="mt-8 text-sm leading-relaxed text-ink/65">
          ...
          ? KIND_ORDER.map((kind) => {
              ...
              return (
                <section key={kind} data-testid={`group-${kind}`} className="mt-10">
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-ink/55">{KIND_BLURBS[kind]}</p>
...
function GameCard({ game }: { game: ListedGame }) {
  ...
  return (
    <Link
      ...
      className="block rounded-2xl border border-ink/10 bg-ink/4 p-4"
    >
      ...
      <p className="mt-1 text-xs text-ink/55">each</p>

      {game.hint ? (
        <p
          data-testid={`hint-${game.publicId}`}
          className="mt-3 border-l-2 border-gold/45 pl-3 text-sm leading-relaxed text-ink/70"
        >
          {game.hint}
        </p>
      ) : null}
      ...
      {locked ? (
        <p
          ...
          className="mt-3 rounded-xl bg-ink/6 px-3 py-2 text-xs leading-relaxed text-ink/70"
        >
      ...
      <p className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs tabular-nums text-ink/55">
...
function Empty() {
  return (
    <div data-testid="games-empty" className="mt-12 rounded-2xl bg-ink/5 p-4">
      <p className="text-sm leading-relaxed text-ink/75">There is nothing to earn right now.</p>
